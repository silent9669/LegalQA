...
def staged_data():
    if not os.path.exists(os.path.join(STAGED_DIR, "qa_unique.parquet")):
        pytest.skip("Staged dataset artifacts not present locally.")
    return STAGED_DIR

def test_dataset_integrity_validator(staged_data):
    report = validate_dataset(data_dir=staged_data, schema_path=SCHEMA_PATH)
...
def test_zero_code_in_dataset(staged_data):
    # Strictly zero code files or code directories in dataset package
    subdirs = [d for d in os.listdir(staged_data) if os.path.isdir(os.path.join(staged_data, d))]
    ...
    py_files = glob.glob(os.path.join(staged_data, "**/*.py"), recursive=True)
    sh_files = glob.glob(os.path.join(staged_data, "**/*.sh"), recursive=True)
    assert len(py_files) == 0, f"Found .py files in dataset: {py_files}"
    assert len(sh_files) == 0, f"Found .sh files in dataset: {sh_files}"

def test_parquet_referential_integrity(staged_data):
    qa_df = pd.read_parquet(os.path.join(staged_data, "qa_unique.parquet"))
    legal_df = pd.read_parquet(os.path.join(staged_data, "legal_chunks.parquet"))
    citations_df = pd.read_parquet(os.path.join(staged_data, "qa_citations.parquet"))
    labels_df = pd.read_parquet(os.path.join(staged_data, "retrieval_labels.parquet"))
    folds_df = pd.read_parquet(os.path.join(staged_data, "fold_assignments.parquet"))
...
def test_manifest_checksums_match_actual_bytes(staged_data):
    manifest_path = os.path.join(staged_data, "dataset_manifest.json")
    ...
    for fname, meta in manifest.get("files", {}).items():
        fpath = os.path.join(staged_data, fname)
