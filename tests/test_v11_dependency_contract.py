...
def test_stale_v10_package_rejected_by_v11_validator(tmp_path: Path):
    """Verify that a package with runtime_api_version=10 is rejected when expected_api_version=11."""
    runtime = tmp_path / "runtime"
    code = runtime / "code" / "LegalQA"
    code.mkdir(parents=True)
    (runtime / "dataset_manifest.json").write_text(
        json.dumps({"runtime_api_version": 10, "git_sha": VALID_TEST_SHA})
    )
    (code / "code_manifest.json").write_text(
        json.dumps({"runtime_api_version": 10, "git_sha": VALID_TEST_SHA})
    )

    with pytest.raises(RuntimeError, match="runtime_api_version mismatch: found 10, expected 11"):
        validate_runtime_manifests(str(runtime), str(code), expected_api_version=11)


def test_fresh_v11_package_passes_v11_validator(tmp_path: Path):
    """Verify that a fresh package with runtime_api_version=11 passes validation."""
    assert EXPECTED_RUNTIME_API_VERSION == 11
    runtime = tmp_path / "runtime"
    code = runtime / "code" / "LegalQA"
    code.mkdir(parents=True)
    (runtime / "dataset_manifest.json").write_text(
        json.dumps({"runtime_api_version": 11, "git_sha": VALID_TEST_SHA})
    )
    (code / "code_manifest.json").write_text(
        json.dumps({"runtime_api_version": 11, "git_sha": VALID_TEST_SHA})
    )
